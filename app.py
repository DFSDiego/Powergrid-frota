...
# Pegar placa da URL (Novo método Streamlit)
...
# --- FORMULÁRIO ---
# Nota: Não usamos o st.form para as câmeras para permitir a lógica sequencial (uma foto após a outra)
...
if f1:
    ...
    # Só mostra a segunda câmera após a primeira foto ser tirada para evitar conflito no navegador
...
# Botão de Envio (Fora do st.form para funcionar com a lógica de câmera)
...
with st.expander("🔐 Acesso Administrativo"):
    ...
    if senha == SENHA_ADMIN:
        if os.path.exists(ARQUIVO_CSV):
            ...
            if not df_adm.empty:
                linha_idx = st.selectbox("Selecione o registro para ver as fotos:", df_adm.index, format_func=lambda x: f"ID {x} - {df_adm.loc[x, 'Data']} - {df_adm.loc[x, 'Funcionario']}")
                ...
                if st.button("🗑️ Excluir este Registro"):
                    ...
                    st.success("Registro e fotos excluídos!")
                    st.rerun()
        else:
