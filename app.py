...
try:
    placa = st.query_params.get("placa", "GIN1G73") # Valor padrão para teste
except:
...
# --- INSTRUÇÃO DE CÂMERA ---
st.caption("🔔 Se o navegador pedir, clique em 'PERMITIR' para usar a câmera.")
...
with st.form("form_powergrid", clear_on_submit=False):
    ...
    # Itens do checklist conforme seu exemplo
    itens = {
        "Óleo do Motor": "oleo",
        "Faróis": "farois",
        "Lanternas": "lanternas",
    ...
    st.markdown("---")
    defeitos = st.text_area("🛠️ Outros Defeitos / Observações", placeholder="Ex: Farol frontal trincado...")
    
    st.write("### 📸 Fotos (Obrigatório)")
    f1 = st.camera_input("Foto do Painel (KM e Combustível)")
    f2 = st.camera_input("Foto Frontal do Veículo")
...
if submit:
    if not nome or not f1:
        ...
        # 2. Salvar no CSV (Lógica de Concat para evitar corrupção)
        novo_reg = {
        ...
        # 3. Montar Mensagem WhatsApp (EXATAMENTE COMO SOLICITADO)
        msg_wpp = (
...
with st.expander("🔐 Acesso Administrativo"):
    if st.text_input("Senha", type="password") == SENHA_ADMIN:
        if os.path.exists(ARQUIVO_CSV):
            ...
            # Selecionar registro para ver fotos
            if not df_adm.empty:
                linha_idx = st.selectbox("Selecione o ID do registro para ver as fotos:", df_adm.index)
                fotos_registro = str(df_adm.loc[linha_idx, "Fotos"]).split(", ")
                ...
                for i, f_nome in enumerate(fotos_registro):
                    ...
                    if os.path.exists(caminho_img) and f_nome != "nan":
                        with cols_fotos[i]:
                            st.image(caminho_img, caption=f"Foto {i+1}")
                
                st.write("---")
                st.write("### 🗑️ Excluir Registro")
                if st.button("Confirmar Exclusão deste Registro"):
                    # Apagar fotos físicas
                    for f_nome in fotos_registro:
                        caminho_f = os.path.join(PASTA_FOTOS, f_nome.strip())
                        if os.path.exists(caminho_f): os.remove(caminho_f)
                    # Remover do CSV
